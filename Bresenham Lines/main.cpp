...
int main()
{
    ...
    // Shaders
    // ------------------------------------
    // Build shaders (compile + link manually or use helper functions)
    ...
    // Generate Bresenham line points (moved to render loop)
    std::vector<float> vertices = bresenhamLine(100, 100, 700, 500, SCR_WIDTH, SCR_HEIGHT);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        int num_points = 500; // number of points in the line

        float amplitude = 100.0f;  // pixels
        float frequency = 0.02f;   // controls wavelength
        float phase = currentFrame;            // animates the wave

        std::vector<float> vertices;
        for (int i = 0; i < num_points; ++i) {
            float x = x_start + i * (x_end - x_start) / float(num_points - 1);
            float y = SCR_HEIGHT / 2 + amplitude * sin(frequency * x + phase); // center vertically
